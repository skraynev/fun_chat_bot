...
class EkiGame:
    tasks = {
        ...
        6: Task(
            "Сочинить стих с рифмой на каждое из слов "
            "(пример: добрый дядя полиглот - утром выпил весь *** (компот))",
            120,
            4,
        ),
                ...
                with open(join(path, file), "r", encoding="utf8") as fi:
                    ...
                    for task_id in data:
                        self.task_to_words[task_id].extend(
                            [(theme, word) for word in data[task_id]]
                        )
    ...
    def del_player(self, user_id):
        if user_id in self.players:
            if len(self.players) == 2:
                return (
                    "Невозможно покинуть игру. Достигнуто минимальное "
                    "количество игроков."
                )
            elif user_id == self.context["player_id"] and self.status == IN_PROGRESS:
                return (
                    "Невозможно покинуть игру. Текущий игрок должен либо "
                    "пропустить вопрос, либо ответить."
                )
            else:
                extra_msg = (
                    self.next_turn if user_id == self.context["player_id"] else ""
                )
    ...
    def get_task(self):
        if self.status == IN_PROGRESS:
            return (
                (
                    "Невозможно запустить задачу - предыдущий вопрос еще "
                    "не закончился."
                ),
                None,
                None,
