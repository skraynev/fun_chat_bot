...
ВАЖНО:
 - Для корректной работы необходимо один раз отправить сообщение любого содержания боту: @%s
...
def join_to_game(message):
    ...
    bot.send_message(chat_id, game_cls.add_player(user.id, user.username, user.first_name))
    try:
        bot.send_message(user.id, 'Вы присоединились к игре. В этот чат будут присылаться слова для заданий.')
        ...
        if (json_res['error_code'] == 403
                and json_res['description'] == "Forbidden: bot can't initiate conversation with a user"):
            bot.send_message(chat_id, 'Отправьте любое сообщение боту @%s. Затем выполните /drop_me и /join команды.' % bot.get_me().username)
        else:
...
def leave_game(message):
    ...
    if not chat_id:
        return
    game_cls = eki_games[chat_id]
    bot.send_message(chat_id, game_cls.del_player(message.from_user.id), parse_mode="Markdown")
...
def send_question(message):
    ...
    if len(game_cls.players) < 2:
        bot.send_message(chat_id, "Недостаточно игроков для игры. Минимально разрешенное количество - 2.")
        return
    ...
    if err_msg is None:
        try:
            ...
            bot.send_message(chat_id, 'Задание отправлено игроку: %s' % game_cls.context['player'])
...
def who_is_next(message):
    ...
    if not chat_id:
        return
    game_cls = eki_games[chat_id]
    run_msg = 'Когда игрок будет готов запустите таймер и получите задание выполнив команду - /go'
